...
# Function to refresh and slightly randomize dashboard metrics
...
api_key_input = st.sidebar.text_input(
    ...
    help="Enter your Google Gemini API Key to enable real-time AI responses. If blank, simulated fallback responses will be used."
)
...
# Sidebar Operations Trigger
st.sidebar.markdown("---")
if st.sidebar.button("🔄 Refresh Stadium Metrics", help="Simulate updated stadium telemetry data."):
...
if navigation == "🏟️ Fan Assistant":
    ...
    def generate_chat_response(prompt_text):
        ...
        try:
            # Prepare recent message history format for the SDK
            contents = []
            ...
            # If the current prompt isn't appended yet
            ...
            return f"⚠️ Connection/API Error: {str(e)}. Please check your API key status."
    ...
    if st.button("🗑️ Clear Chat History"):
        st.session_state.chat_history = [
            {"role": "assistant", "content": "Welcome to MetLife Stadium! I am your AI Fan Assistant for the FIFA World Cup 2026. How can I help you today? ⚽"}
        ]
        st.rerun()
...
# ==========================================
    ...
    with plot_col2:
        # Plot 2: Waste components (circular donut chart)
    ...
    if st.button("⚡ Run AI Operations Analysis"):
        with st.spinner("Analyzing live feeds and log registers..."):
            ...
            if not client:
                ...
                try:
                    ...
                    ai_briefing = f"⚠️ API Error performing analysis: {str(e)}"
...
# ==========================================
    ...
    with col_form:
        ...
        with st.form("new_incident_form", clear_on_submit=True):
            category = st.selectbox("Category", ["Facility", "Medical", "Technical", "Safety/Security", "Other"])
            location = st.text_input("Location / Seating Section", placeholder="e.g. Sec 102, Row B, Seat 4")
            severity = st.select_slider("Severity Level", options=["Low", "Medium", "High", "Critical"])
            description = st.text_area("Issue Description", placeholder="Provide detail (e.g., wet floor, broken scanner, fan injury)...")
            reporter = st.text_input("Reporter ID / Unit", placeholder="e.g. Volunteer #18, Usher Unit C")
            
            submitted = st.form_submit_form_button("🚨 Submit Incident Report")
    ...
    with col_log:
        ...
        if st.session_state.incidents.empty:
                    ...
                    with st.spinner(f"Analyzing SOPs for {selected_incident_id}..."):
                        ...
                        if not client:
                            ...
                            try:
                                ...
                                action_plan = f"⚠️ API Error generating action plan: {str(e)}"
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("🗑️ Clear Incident Log"):
...
st.markdown(
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; color: #64748b; font-size: 0.8rem; padding: 0 10px;">
        ...
        <span>Version 1.0.0 (Pre-match build)</span>
    </div>
    """,
    unsafe_allow_html=True
)
